    ...
    <div className="space-y-6">
      {!response ? (
        <div className="bg-white space-y-4">
          <div style={{ display: 'flex', gap: '1rem', borderBottom: '1px solid #EADCC9', paddingBottom: '0.75rem', marginBottom: '1rem' }}>
            <button 
              className={mode === 'text' ? 'btn-toggle-bionic-active' : 'btn-toggle-inactive'}
            ...
            <button 
              className={mode === 'url' ? 'btn-toggle-spacing-active' : 'btn-toggle-inactive'}
              onClick={() => setMode('url')}
              style={{ padding: '0.5rem 1rem' }}
            >
          ...
          {mode === 'text' ? (
            ...
            <div style={{ padding: '0.5rem 0 1.5rem 0' }}>
              <input
                ...
                style={{ width: '100%', padding: '1rem', border: '1px solid #D2B48C', borderRadius: '0.5rem', background: '#FAFAFA', boxSizing: 'border-box', fontSize: '1rem' }}
              />
            </div>
          )}
          ...
          <div className="flex-wrap" style={{ marginTop: '1.5rem' }}>
            <label>
              ...
              />
              <span>Enable OpenAI Cognitive Restructuring (Highly Recommended for Web Scrapes)</span>
            </label>
            <button
            ...
            >
              <span>{loading ? 'Processing Matrix Pipeline...' : 'Scrape, Simplify & Render'}</span>
            </button>
          </div>
        </div>
      ) : (
        <div className="space-y-6">
          <div className="bg-white flex-wrap">
            <div className="flex-gap-4">
              <button
                ...
                className={bionicEnabled ? 'btn-toggle-bionic-active' : 'btn-toggle-inactive'}
              >
                Bionic Fixation Points
              </button>
              <button
                onClick={() => setIsolateInversions(!isolateInversions)}
                className={isolateInversions ? 'btn-toggle-inversion-active' : 'btn-toggle-inactive'}
              >
                Highlight Mirror Letters (b/d/p/q)
              </button>
              <button
                onClick={() => setLargeSpacing(!largeSpacing)}
                className={largeSpacing ? 'btn-toggle-spacing-active' : 'btn-toggle-inactive'}
              >
                Accessible Letter Spacing
              </button>
            </div>
            <button
              onClick={() => { setResponse(null); setText(''); setUrl(''); }}
              className="btn-clear"
            >
              Clear Workspace
            </button>
          ...
          <div className="output-viewport">
            <div className="reader-article-header">
              {response.site_name && <span className="reader-site-tag">{response.site_name}</span>}
              {response.title && <h2 className="reader-hero-title">{response.title}</h2>}
            </div>
            ...
            >
              {response.word_matrix.map((token, idx) => {
                const needsHighlight = isolateInversions && token.inversion_risk;
                return (
                  <span
                    key={idx}
                  ...
                  >
                    {bionicEnabled ? (
                      <span dangerouslySetInnerHTML={{ __html: token.bionic_html }} />
                    ) : (
                      token.original
                    )}
                  </span>
